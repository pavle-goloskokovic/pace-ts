...
class Bar {
  ...
  render() {
    ...
    for (let key of ['webkitTransform', 'msTransform', 'transform']) {
...
const shouldIgnoreURL = function(url) {
  for (let pattern of Array.from(options.ajax.ignoreURLs)) {
...
class XHRRequestTracker {
  constructor(request) {
    ...
    if (window.ProgressEvent != null) {
      ...
      for (let event of ['load', 'abort', 'timeout', 'error']) {
...
class ElementMonitor {
  constructor(options) {
    ...
    for (let selector of Array.from(options.selectors)) {
...
(init = function() {
  ...
  for (let type of ['ajax', 'elements', 'document', 'eventLag']) {
